...
int main (void) {
	...
	while(1) {
		...
		if (c != '\0'){
			...
			for (unsigned char ucCounter = 0; ucCounter < SAMPLES_PER_MEASUREMENT; ucCounter++){
				adc_start_conversion(&MY_ADC, MY_ADC_CH);
				adc_wait_for_interrupt_flag(&MY_ADC, MY_ADC_CH);
				result = adc_get_signed_result(&MY_ADC, MY_ADC_CH);
				oversampled_result += result;
			}
			oversampled_result = (oversampled_result/SAMPLES_PER_MEASUREMENT);
			oversampled_result = (REF_VOLTAGE_mV/2048.0)*oversampled_result;
			printf("%f\n",oversampled_result);
		}
		else{
			printf("Nothing\n");
...
//-----------------------------------------
//PA.1 1.720V, PA.5 1.562V, napiêcie odniesienia 158mV
// 295-301 bez usredniania, gain = 1
// Przeliczyæ ile mV przypada na jeden LSB i porównaæ z wartoœci¹ wynikaj¹c¹ z dokumentacji
// 145 mV odczyt
