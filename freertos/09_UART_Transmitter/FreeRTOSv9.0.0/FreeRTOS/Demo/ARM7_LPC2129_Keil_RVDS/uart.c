...
__irq void UART0_Interrupt (void) {
   ...
   {
				if (xQueueReceiveFromISR( xQueueUART, &cCharacterToSend,&xTaskWokenByReceive)==pdTRUE){ 
...
void UART_InitWithInt(unsigned int uiBaudRate){
	...
	xQueueUART = xQueueCreate( 20, sizeof(char) );
...
void Uart_PutString(char * pcString){
	 ...
	 if(xSemaphoreTake(xBinarySemaphoreTx, portMAX_DELAY)==pdTRUE){
      ...
      cCharacterToSend = *pcString;
			...
			while(*pcString != NULL){
				pcString++;
				xQueueSendToBack( xQueueUART , pcString, NULL);	 
			}
			U0THR = cCharacterToSend;
		}
}
